...
export function AboutPillars({ section }: AboutPillarsProps) {
  ...
  const badge = sectionString(section, 'badge') || t('about.pillars.badge');
  const title = sectionString(section, 'title') || section?.title || t('about.pillars.title');
    ...
    <section className="py-12 sm:py-14 border-t border-border bg-bg-card">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-8">
          ...
          <h2 className="text-[clamp(1.4rem,2.4vw,1.8rem)] font-extrabold text-white tracking-[-0.02em]">
            {title}
          </h2>
        </div>
        ...
        <div className="grid gap-3.5 sm:grid-cols-2 lg:grid-cols-3">
          {pillars.map((p, i) => {
            const c = colorMap[p.iconColor as keyof typeof colorMap] ?? colorMap.green;
            return (
              <PillarCard key={i} delay={i * 100}>
                <div className={cn(
                  'group rounded-lg border border-border bg-bg-elevated p-4 transition-all duration-[0.45s] ease-premium will-change-transform',
                  'hover:-translate-y-1 hover:scale-[1.01]',
                  `hover:${c.border} hover:${c.shadow}`,
                )}>
                  <div className={cn('mb-2.5 flex h-9 w-9 items-center justify-center rounded-lg', c.bg, c.text)}>
                    {icons[p.iconColor] ?? icons.green}
                  </div>
                  <h3 className="text-[0.9rem] font-bold text-white mb-1">{p.title}</h3>
