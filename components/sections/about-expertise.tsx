...
function StatItem({ value, suffix, label }: { value: number; suffix: string; label: string }) {
  ...
  return (
    <div className="flex-1 py-4 px-3 text-center border-r border-border last:border-r-0">
      <div className="inline text-[1.4rem] font-extrabold text-white">
        <span ref={counter.ref}>0</span>
      </div>
      {suffix === 'kW' ? (
        <span className="text-sm font-bold text-green">{suffix}</span>
      ) : suffix === '%' ? (
        <span className="text-sm font-bold text-green">{suffix}</span>
      ) : suffix === '+' ? (
        <span className="text-sm font-bold text-green">+</span>
      ) : null}
      <span className="block text-[0.65rem] text-gray-400 uppercase tracking-[0.06em] mt-1">{label}</span>
    </div>
  );
}
...
export function AboutExpertise() {
  ...
  return (
    <section className="py-12 sm:py-14 border-t border-border bg-bg-card">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <FadeIn>
          <h2 className="text-[clamp(1.4rem,2.4vw,1.8rem)] font-extrabold text-white text-center tracking-[-0.02em] mb-2">
            {t('about.expertise.title')}
          </h2>
          <p className="text-center text-sm text-gray-400 max-w-[560px] mx-auto mb-8 leading-relaxed">
        ...
        {/* Stats segmented bar */}
        <FadeIn>
          <div className="flex justify-center mb-8 rounded-xl border border-border bg-bg-elevated overflow-hidden">
            {stats.map((stat, i) => (
        ...
        <div className="grid gap-3.5 sm:grid-cols-2 lg:grid-cols-4">
          {expertise.map((item, i) => (
            <FadeIn key={item.title} delay={i * 100}>
              <div
                className={cn(
                  'group rounded-lg border border-border bg-bg-primary p-4 transition-all duration-[0.45s] ease-premium will-change-transform',
                  'hover:-translate-y-1 hover:scale-[1.01]',
                  'hover:border-green/25 hover:shadow-[0_16px_40px_rgba(0,0,0,0.35),0_0_20px_rgba(34,197,94,0.12)]',
                )}
              >
                <div className={cn('mb-2.5 flex h-9 w-9 items-center justify-center rounded-lg', item.iconBg)} style={{ color: item.iconColor }}>
                  {icons[item.icon as keyof typeof icons]}
                </div>
                <h3 className="text-[0.85rem] font-bold text-white mb-1.5">{item.title}</h3>
