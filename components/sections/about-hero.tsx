...
export function AboutHero({ section }: AboutHeroProps) {
  ...
  const buttons =
    sectionButtons(section).length > 0
      ...
      : [{ label: t('hero.about.cta1'), href: '#innovation', variant: 'primary' }];
  ...
  return (
    <section className="relative w-screen left-1/2 -translate-x-1/2 min-h-[55vh] flex items-center justify-center overflow-hidden bg-bg-primary">
      ...
      <div className="relative z-10 w-full max-w-3xl px-6 sm:px-8 lg:px-10 py-16 sm:py-20 text-center">
        ...
        <div className="animate-fade-up flex flex-col sm:flex-row items-center justify-center gap-3" style={{ animationDelay: '0.55s', animationFillMode: 'both' }}>
          {buttons.map((btn) => {
            const isPrimary = btn.variant === 'primary' || btn.variant === 'whatsapp';
            return (
              <a
                ...
                className={
                  isPrimary
                    ? 'inline-flex items-center gap-2 rounded-full bg-green px-7 py-3 text-sm font-semibold text-bg-primary transition-all duration-300 hover:bg-green-dark hover:shadow-glow active:scale-[0.98]'
                    : 'inline-flex items-center gap-2 rounded-full border border-white/20 bg-white/5 px-7 py-3 text-sm font-semibold text-white transition-all duration-300 hover:bg-white/10 hover:border-white/35 active:scale-[0.98]'
                }
              >
                {btn.label}
