...
export function HomeProjectsPreview({ section, projects = [] }: HomeProjectsPreviewProps) {
  const { t } = useTranslation();

  const title = section?.title || t('home.projects.title');
  const titleHighlight = sectionString(section, 'titleHighlight', t('home.projects.titleHighlight'));
  const description = section?.description || t('home.projects.description');
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <Reveal>
          <div className="flex items-end justify-between mb-10 flex-wrap gap-4">
            ...
            >
              {button?.label || t('home.projects.link')}
            ...
            <Reveal key={project.id} delay={i * 80}>
              <div className="group min-w-[280px] sm:min-w-0 overflow-hidden rounded-2xl border border-border bg-bg-card transition-all duration-500 hover:-translate-y-2 hover:scale-[1.02] hover:border-green/25 hover:shadow-card-hover">
                <div className="relative h-44 flex items-center justify-center bg-bg-elevated overflow-hidden">
                  {project.image ? (
                    <Image src={project.image} alt={project.title} fill className="object-cover transition-all duration-500 group-hover:scale-110" />
                  ) : (
                ...
                <div className="p-4">
                  ...
                  <p className="mt-1.5 text-xs text-gray-500">{project.location}{project.power ? ` — ${project.power}` : ''}</p>
                </div>
              </div>
            </Reveal>
          ))}
