...
import { sectionButton } from '@/lib/section-utils';

interface HomeCTAProps {
  section?: PageSection;
}

export function HomeCTA({ section }: HomeCTAProps) {
  const { t } = useTranslation();

  const title = section?.title || t('home.cta.title');
  const description = section?.description || t('home.cta.description');
  ...
  const label =
    button?.label || t('common.whatsapp');
            ...
            <div className="relative z-10 mx-auto max-w-xl">
              ...
              <div className="mt-8">
                <a
                  ...
                  className={`group inline-flex items-center gap-2 rounded-full px-7 py-3 text-sm font-semibold transition-all duration-300 active:scale-[0.98] ${
                    isWhatsapp
                      ? 'bg-[#25D366] text-white hover:shadow-[0_0_30px_rgba(37,211,102,0.4)]'
                      : 'bg-green text-bg-primary hover:bg-green-dark hover:shadow-glow'
                  }`}
                >
                  {isWhatsapp && (
