...
import { sectionString, sectionHighlights, sectionImage } from '@/lib/section-utils';

interface HomeAboutPreviewProps {
  section?: PageSection;
}

export function HomeAboutPreview({ section }: HomeAboutPreviewProps) {
  const { t } = useTranslation();

  const badge = sectionString(section, 'badge', t('home.about.badge'));
  const title = section?.title || t('home.about.title');
  const titleHighlight = sectionString(section, 'titleHighlight', t('home.about.titleHighlight'));
  const paragraph1 = section?.description || t('home.about.paragraph1');
  const paragraph2 = sectionString(section, 'paragraph2', t('home.about.paragraph2'));
  ...
  const highlightItems =
    highlights.length > 0
      ? highlights
      : [t('home.about.highlight1'), t('home.about.highlight2'), t('home.about.highlight3')];
