...
import { sectionStats } from '@/lib/section-utils';
...
function StatCard({ stat, index }: { stat: { value: number; suffix: string; label: string; color: string; glow: string; icon: React.ReactNode }; index: number }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-center">
        ...
        <span className="text-[0.65rem] text-gray-500 uppercase tracking-[0.08em] mt-1">{t(stat.label, { defaultValue: stat.label })}</span>
      </div>
    </div>
  );
}
...
export function ProjectsStats({ section }: ProjectsStatsProps) {
  const dbStats = sectionStats(section);
  const stats = dbStats.length > 0
    ? dbStats.map((s, i) => {
        const p = palette[i % palette.length];
        return { value: s.value, suffix: s.suffix, label: s.label, ...p };
      })
