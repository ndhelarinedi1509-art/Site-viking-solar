          ...
          {messages.map((msg) => (
            <div key={msg.id} className="p-4 space-y-3">
              <div className="flex items-center justify-between gap-2">
                <span
                  className={`inline-block px-2.5 py-1 rounded-full text-xs font-medium ${
                    msg.status === 'new' ? 'bg-green/10 text-green' : 'bg-white/5 text-gray-400'
                  }`}
                >
                  {msg.status === 'new' ? t('admin.messages.new') : t('admin.messages.read')}
                </span>
              ...
              <div className="flex items-center justify-between gap-2">
                ...
                <button
                  onClick={() => handleOpen(msg.id)}
                  className="h-9 px-4 rounded-lg bg-green/10 text-green text-xs font-semibold hover:bg-green/20 transition-all duration-200 flex items-center gap-1.5 flex-shrink-0"
                >
                  <Eye className="h-3.5 w-3.5" />
                  {t('admin.messages.open')}
                </button>
              </div>
              <p className="text-sm text-gray-400 leading-relaxed">{msg.text}</p>
            </div>
          ))}
        </div>
      </div>
